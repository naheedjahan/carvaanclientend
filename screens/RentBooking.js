...
class RentBooking extends React.Component {
  ...
  handleRentProcess = async () => {
    ...
    console.log(vehicleId);
    const token = await AsyncStorage.getItem('Customertoken');
    fetch(baseUrl + 'api/customer/request/' + vehicleId, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-auth-token': token,
      ...
      .then(async (data) => {
        try {
          if (!data.errors) {
            console.log(data);
            const id = data;
            console.log('check request id' + id);
            // this.props.navigation.navigate('login');
            this.props.navigation.navigate('popUp', { RequestId: id });
            // this.props.navigation.navigate('EndVehicleBooking', {
            //   vehicleId: vehicleId,
            //   bookingId: id,
            // });
  ...
  render() {
    const manf = this.props.route.params.manf;
