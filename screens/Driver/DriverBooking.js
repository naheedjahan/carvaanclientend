...
class DriverBooking extends React.Component {
  ...
  handleRentProcess = async () => {
    ...
    //-----------------------------------------------------------
    const driverId = this.props.route.params.driverId;
    ...
    fetch(baseUrl + 'api/customer/Drequest/' + driverId, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-auth-token': token,
      ...
      .then(async (data) => {
        try {
          if (!data.errors) {
            ...
            const bill = 300 * d;
            this.props.navigation.navigate('DriverBookingStatistics', {
              bill: bill,
              img: image,
              name: name,
              address: address,
  ...
  render() {
    ...
    const Id = this.props.route.params.driverId;
