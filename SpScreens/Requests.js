...
function Item({ item }) {
  ...
  return (
      ...
      <TouchableOpacity style={styles.listItem} onPress={() => navigation.navigate('MyBookingDetails', {
        item
     })}>
      ...
      <Text style={{fontWeight:"bold",marginTop:10}}>On: </Text>
        <Text size={13}style={{color:"green"}}>{item.date}</Text>
        {/* <Rating
              type='star'
              imageSize={15}
              readonly
              ...
              startingValue={2}
            /> */}
            <Text  size={13} style={{fontWeight:"bold", marginTop:7}}> For: </Text> 
            <Text  size={13} style={{color:"green"}}> {item.days} Days</Text> 
      </View>
...
constructor(props){
  ...
  componentDidMount() 
      ...
      getData = async () => {
        ...
        const response = await fetch('http://192.168.1.153:5000/api/owner/viewOwnerBookings', {
          method: 'GET',
          headers: {
            'x-auth-token': token,
          },
