...
function Item({ item }) {
  ...
  return (
      ...
      <TouchableOpacity style={styles.listItem} onPress={() => navigation.navigate('CarDetails', {
        item
     })}>
     
     <Image source={{uri:item.image}}  style={{width:180, height:100,borderRadius:1}} />
      <View style={{alignItems:"center",flex:1}}>
        <Text style={{fontWeight:"bold",marginTop:10}}>{item.manufacturer}</Text>
        <Rating
              type='star'
              imageSize={15}
              readonly
...
constructor(props){
  ...
  componentDidMount() {
      ...
      getData = async () => {
        ...
        const response = await fetch('http://192.168.1.153:5000/api/owner/myvehicles', {
          method: 'GET',
          headers: {
            'x-auth-token': token,
          },
