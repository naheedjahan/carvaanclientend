...
export default class AddSubscription extends React.Component {
  ...
  onSubmit = async (creditCardData) => {
    ...
    const booking = this.props.route.params.bookingId;
    const information = {
      card: {
    ...
    if (driver == 'fullPayment') {
      // this.fullPayment(booking);
      this.fullDriverPayment(booking);
    }
  };
  fullPayment = async (bookingId) => {
    ...
    fetch(baseUrl + 'api/customer/serviceEnd/' + bookingId, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-auth-token': token,
  ...
  fullDriverPayment = async (BookingId) => {
    //-----------------------------------------------------------
